...
- (UIImage *)convertToTransparentImage {
    ...
    for(int i=0; i<lengthSource; i+=4)
    {
        ...
        float alpha = 1.0-(float)minVal/255.0;
        
        raw[i]      = (char)((raw[i]-(1.0-alpha)*255.0)/alpha);
        raw[i+1]	= (char)((raw[i+1]-(1.0-alpha)*255.0)/alpha);
        raw[i+2]	= (char)((raw[i+2]-(1.0-alpha)*255.0)/alpha);
        raw[i+3]    = (char)(alpha*255.0);
    }
    
    CFDataRef newData           = CFDataCreate(NULL, raw, lengthSource);
    CGDataProviderRef provider  = CGDataProviderCreateWithCFData(newData);
    
    CGImageRef newImg           = CGImageCreate(CGImageGetWidth(imgSource),
                                                CGImageGetHeight(imgSource),
                                                CGImageGetBitsPerComponent(imgSource),
                                                CGImageGetBitsPerPixel(imgSource),
                                                CGImageGetBytesPerRow(imgSource),
                                                CGImageGetColorSpace(imgSource),
                                                kCGBitmapByteOrderDefault | kCGImageAlphaLast,
                                                provider,
                                                NULL, true, kCGRenderingIntentDefault);
    UIImage *retImage = [UIImage imageWithCGImage:newImg];
    CFRelease(m_DataRef1);
    CFRelease(newData);
